...
def _build_power(circuit: Circuit, nets: dict[str, Net]) -> None:
    ...
    for index, pin in (("1", "A5"), ("2", "B5")):
        rd = _rc(circuit, f"R{index}", "5.1k", "RC0603FR-075K1L")
    ...
    shield_resistor = _rc(circuit, "R3", "1M", "RC0603FR-071ML")
    shield_capacitor = two_pin(
        circuit, "C12", "4.7n 1kV", "Capacitor_SMD:C_1206_3216Metric",
        mpn="GRM31BR73A472KW01L", unit_cost_eur=0.05,
    )
    ...
    vpcc_top = _rc(circuit, "R4", "330k 1%", "RC0603FR-07330KL")
    ...
    for ref, pin, value, mpn in (
        ("R6", "12", "25k", "RC0603FR-0725KL"),
        ("R7", "13", "2.0k", "RC0603FR-072KL"),
    ):
    ...
    for ref, name in (("R8", "CHG_PG_N"), ("R9", "CHG_STAT2"), ("R10", "CHG_STAT1_N")):
        pullup = _rc(circuit, ref, "10k", "RC0603FR-0710KL")
        _connect(nets["3V3"], pullup, "1")
        _connect(nets[name], pullup, "2")
    for ref, name, value, mpn in (
        ("C13", "USB_VBUS", "4.7u 10V", "GRM21BR61A475KA73L"),
        ("C14", "SYS", "10u 10V", "GRM21BR61A106KE19L"),
        ("C15", "BAT+", "4.7u 10V", "GRM21BR61A475KA73L"),
    ):
        capacitor = two_pin(
    ...
    power_enable_pull = _rc(circuit, "R11", "1M", "RC0603FR-071ML")
    ...
    buck_pg_pullup = _rc(circuit, "R12", "100k", "RC0603FR-07100KL")
    ...
    fb_bottom = _rc(circuit, "R14", "91k", "RC0603FR-0791KL")
    ...
    for ref, net, value in (("C1", "SYS", "22u"), ("C2", "3V3", "22u"), ("C3", "3V3", "22u")):
        capacitor = two_pin(
            ...
            mpn="GRM21BR61A226ME44L", unit_cost_eur=0.05,
        )
...
def _build_led_rail(circuit: Circuit, nets: dict[str, Net]) -> Net:
    ...
    led_fb_bottom = _rc(circuit, "R16", "100k", "RC0603FR-07100KL")
    ...
    led_data_pulldown = _rc(circuit, "R19", "100k", "RC0603FR-07100KL")
...
def _build_mcu(circuit: Circuit, nets: dict[str, Net]) -> None:
    ...
    en_pull = _rc(circuit, "R20", "10k", "RC0603FR-0710KL")
    ...
    led_en_pulldown = _rc(circuit, "R24", "100k", "RC0603FR-07100KL")
    _connect(nets["LED_EN"], led_en_pulldown, "1")
    _connect(nets["GND"], led_en_pulldown, "2")
    button_pullup = _rc(circuit, "R25", "10k", "RC0603FR-0710KL")
    ...
    rclk_pulldown = _rc(circuit, "R26", "100k", "RC0603FR-07100KL")
...
def _build_reader(circuit: Circuit, nets: dict[str, Net]) -> None:
    ...
    core_cap = _rc(circuit, "C20", "1u 10V", "GRM155R61A105KE15D")
    ...
    for ref, net in (("C21", vmid), ("C22", vdhf)):
        stabilizer = _rc(circuit, ref, "100n", "CC0402KRX7R7BB104")
        _connect(net, stabilizer, "1")
        _connect(nets["GND"], stabilizer, "2")
    supply_caps = (
        ("C23", "3V3", "100n", "CC0402KRX7R7BB104"),
        ("C24", "3V3", "1u 10V", "GRM155R61A105KE15D"),
        ("C25", "3V3", "2.2u 10V", "GRM155R61A225KE11D"),
        ("C26", "3V3", "100n", "CC0402KRX7R7BB104"),
    )
    ...
    for ref, clk_net, crystal_net in (("R27", clk1, xti), ("R28", clk2, xto)):
        series = _rc(circuit, ref, "100R", "RC0603FR-07100RL")
    ...
    for ref, crystal_net in (("C31", xti), ("C32", xto)):
        load_capacitor = _rc(circuit, ref, "10p C0G", "GCM1555C1H100GA16D")
    ...
    emc_inductor = two_pin(
        ...
        mpn="LQW21FN471K70L", unit_cost_eur=0.06,
    )
    ...
    rx_series_r = _rc(circuit, "R29", "1k", "RC0603FR-071KL")
    rx_series_c = _rc(circuit, "C37", "100p C0G", "GRM1555C1H101JA01D")
    ...
    rx_bias = _rc(circuit, "R30", "1k", "RC0603FR-071KL")
...
def build_hub() -> Circuit:
    ...
    for display in range(2):
        connector = _connector(circuit, f"J{display + 5}", display_names, "SM07B-GHS-TB", 0.35)
        display_nets = (
    ...
    for bar in range(2):
        connector = _connector(circuit, f"J{bar + 7}", ("LED_5V", "GND", "DATA_IN", "DATA_OUT"), "SM04B-GHS-TB", 0.30)
    ...
    uart = _connector(circuit, "J9", ("3V3", "GND", "UART_TX", "UART_RX"), "SM04B-GHS-TB", 0.30)
    for connector_pin, name in enumerate(("3V3", "GND", "UART_TX", "UART_RX"), start=1):
        _connect(nets[name], uart, str(connector_pin))

    button = _connector(circuit, "J10", ("BUTTON_N", "GND"), "SM02B-GHS-TB", 0.25)
    ...
    srclr_pull = _rc(circuit, "R31", "10k", "RC0603FR-0710KL")
