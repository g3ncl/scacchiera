...
Fourteen is what a hand-populated bar holds. JLCPCB will not assemble a 120 x 8.5 mm outline, so the
LED must be a package whose pads an iron can reach rather than one hiding them under its body, and
such a package is wider: at the chosen part's 7.30 mm courtyard, with the 4-pin JST GH taking
9.46 mm of exclusive length, fourteen fit. The diffuser standoff should be around half the 7.30 mm
pitch so the bar reads as a bar rather than as discrete dots. See
