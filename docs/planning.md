...
- [ ] V1 component and library proof: fitted-board sourcing is complete, but the external display
  ...
  **preliminary** (2025-08-07), a release blocker on its own terms. And the module's
  ...
  no adapter fixes that. The Micro-Fit mating housings and 20 AWG female terminals
  ...
  **stops at 20 AWG at 5 A** with no 18 AWG row, while its bundled terminal drawing states 5.0 A
  maximum. The previously fitted 430300038 is the 18 AWG variant that distributors list at
  8.5 A, which is a catalog figure rather than a datasheet one, so the cell link now binds the
  documented 20 AWG row instead: wire becomes exactly 20 AWG and the terminal becomes 43030-0007,
  a cable-side substitution with no layout consequence. Correcting an earlier error, the governing
  comparison is RMS rather than peak, because a connector rating is a 30 degree temperature-rise
  limit and contact heating is I squared R: the battery path is 4.442 A RMS against 5.0 A, about
  11 percent margin, and the 5.871 A figure is a 500 kHz inductor peak that does not heat a
  contact. Two reservations stand. The derating for a two-circuit housing with both circuits
  energized is absent from the filed revision and at 90 percent would cut the margin to roughly one
  percent, and the cell link is the only power interface in the product that puts its whole current
  through one contact pair, against the multiple-contact principle
  [hardware/power-module-interface.md](hardware/power-module-interface.md) already states. Taking
  it to four circuits would give 2.22 A per contact and settle it, at the cost of a power-board
  re-route. The filed copy is also an Octopart mirror of a 2007 revision because Molex's own
  servers did not respond. The same file retires a suspected J2 problem: its three JST GH supply
...
- [x] Power board schematic: `hardware/pcb/power.py` implements the 5 V at 2 A contract with a
  ...
  inductor, and Micro-Fit power connectors. The matching housings and 20 AWG terminals are exact;
