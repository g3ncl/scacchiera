...
- [ ] V1 component and library proof: open on three items now. A custom power board was added on
  ...
  the superseded charger design. The exact audits for the three bound parts were regenerated on
  2026-07-29. The inductor remains a machine-readable blocker because no exact supplier order code
  was verified; catalogue searches returned neighbouring families rather than this MPN.
  [verification/v1-components.yaml](verification/v1-components.yaml) exactly matches 48 unique
  purchased fitted MPN, supplier, order-code, and footprint tuples generated by the four
  authoritative schematics, records NR6045S1R0NT as the sole fitted-part blocker, and separately
  ...
  items and parity issues, mypy passed on 68 files, and all 84 tests passed on 2026-07-29.
...
- [ ] Power board SPICE validation: no corner or fault simulation exists yet. Charger, cell,
  power-path handover and boost behavior remain open under V3.
...
- [x] Hub board layout: `hardware/pcb/hub_layout.py` generates the 2-layer 162 x 46 mm placement,
  ...
  0 violations, 0 unconnected, and 0 schematic parity issues.
- [x] Power board layout: `hardware/pcb/power_layout.py` generates the routed 46 x 32 mm board.
...
possible. V1 is open on its unverified inductor order code and on the two purchased product classes
described above. V2 passes for all four board designs. The lightbar and matrix retain their
