...
the sensing matrix, the hub and the power board. Two further designs,
[the line strip and its spine](hardware/strip.md), are an alternative way of cutting the sensing
plane rather than an addition to the product; exactly one of the two sensing architectures ships.
All six schematics and code-generated layouts pass the current ERC, DRC and schematic-parity
gates. Simulation coverage is partial, and the new
